...
/// <summary>
/// Handler que procesa el comando para reabrir el período de postulaciones de un ciclo.
/// </summary>
/// <remarks>
/// Flujo de ejecución:
/// 1. Carga el ciclo por CycleId; retorna CYCLE_NOT_FOUND si no existe.
/// 2. Delega la validación de la transición al método de dominio Cycle.ReopenApplications().
...
/// </remarks>
public class ReopenApplicationsCommandHandler : IRequestHandler<ReopenApplicationsCommand, Result<CycleDto>>
{
    ...
    public ReopenApplicationsCommandHandler(IApplicationDbContext context)
    {
        _context = context;
    }

    /// <summary>
    /// Procesa el comando de reapertura de postulaciones aplicando las reglas de la máquina de estados.
    /// </summary>
    /// <param name="request">Comando con el CycleId del ciclo a reabrir postulaciones.</param>
    ...
    /// - INVALID_TRANSITION: El ciclo no está en estado ApplicationsClosed.
    /// </remarks>
    public async Task<Result<CycleDto>> Handle(ReopenApplicationsCommand request, CancellationToken cancellationToken)
    {
        ...
        var domainResult = cycle.ReopenApplications();
